...
export default function MonitoringSystem() {
        ...
        filteredInstitutions.forEach((inst) => {
            ...
            (inst.activities || []).forEach((a) => {
                ...
                if (a.observation && a.observation.length > 0) {
                    const obsText = a.observation.map((o) => `${formatDate(o.date_observation)}: ${o.text_observation}`).join("  |  ");
                    activitiesRows.push([
                        {
                            content: `Obs.: ${obsText}`,
                            colSpan: 5,
            ...
            if (equipRows.length > 0) {
                if (y + equipRows.length * 8 + 20 > pageHeight - 15) {
                    doc.addPage();
                    y = margin;
                }
                doc.setFontSize(8);
            ...
            (inst.activities || []).forEach((activity) => {
                ...
                if (sx !== null && ex !== null) {
                    ...
                    doc.roundedRect(
                        Math.max(sx, CHART_X),
                        bY,
                        Math.max(1, Math.min(ex, CHART_X + CHART_W) - Math.max(sx, CHART_X)),
                        bH,
                        0.5,
                        0.5,
                        "F"
    ...
    const calcActivityProgress = (startDate?: string | Date, endDate?: string | Date): number => {
    ...
    const overallAvgProgress = (() => {
        const withActivities = institutions.filter((inst) => inst.activities.length > 0);

        if (withActivities.length === 0) return 0;

        const sum = withActivities.reduce((acc, inst) => {
            const instPct = Math.round(
                inst.activities.reduce((s, a) => s + calcActivityProgress(a.start_date, a.end_date), 0) / inst.activities.length
            );
                    ...
                    <>
                        ...
                        >
                            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
                                ...
                                <Typography sx={{ fontWeight: 700, color: "#1351B4", fontSize: "1.1rem" }}>{overallAvgProgress}%</Typography>
                            </Box>
                            <LinearProgress
                                variant="determinate"
