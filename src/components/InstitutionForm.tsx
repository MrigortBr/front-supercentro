...
export default function InstitutionForm({ institution, onSave, onCancel, onEdit, readOnly = false, allData }: InstitutionFormProps) {
	...
	const [newObsForEdit, setNewObsForEdit] = useState<Omit<ActivityObservation, "id" | "id_activities">>({ date_observation: todayISO, text_observation: "" });
	const [newObsForNew, setNewObsForNew] = useState<Omit<ActivityObservation, "id" | "id_activities">>({ date_observation: todayISO, text_observation: "" });
	...
	const removeObsFromEdit = (idx: number) => {
		...
		if (editingObsInEditIdx === idx) { setEditingObsInEditIdx(null); setNewObsForEdit({ date_observation: todayISO, text_observation: "" }); }
	};
														...
														{(editingActivityData.observation || []).length > 0 && (
															<Box sx={{ display: "flex", flexDirection: "column", gap: 0.5, mb: 1 }}>
																{(editingActivityData.observation || []).map((obs, obsIdx) => (
																	<Box key={obsIdx} sx={{ display: "flex", alignItems: "center", gap: 1, bgcolor: editingObsInEditIdx === obsIdx ? "#fff9e6" : "#f8f9fa", p: 0.75, borderRadius: 1, border: `1px solid ${editingObsInEditIdx === obsIdx ? "#f0ad00" : "#dee2e6"}` }}>
																		...
																		<IconButton size="small" onClick={() => { setEditingObsInEditIdx(obsIdx); setNewObsForEdit({ date_observation: obs.date_observation?.split("T")[0] ?? "", text_observation: obs.text_observation }); }} sx={{ color: "primary.main", p: 0.25 }}>
																			<Pencil size={12} />
																		</IconButton>
																		<IconButton size="small" onClick={() => removeObsFromEdit(obsIdx)} sx={{ color: "error.main", p: 0.25 }}>
																			<Trash2 size={12} />
																		</IconButton>
																	</Box>
																))}
														...
														<Grid container spacing={1} alignItems="flex-end">
															<Grid item xs={12} sm={3}>
																<TextField
																	...
																	onChange={(e) => setNewObsForEdit({ ...newObsForEdit, date_observation: e.target.value })}
																	InputLabelProps={{ shrink: true }}
																/>
															</Grid>
															<Grid item xs={12} sm={7}>
																<TextField
																	...
																	onChange={(e) => setNewObsForEdit({ ...newObsForEdit, text_observation: e.target.value })}
																/>
															</Grid>
															<Grid item xs={12} sm={2}>
																<Button
																	...
																	fullWidth
																	size="small"
																	startIcon={editingObsInEditIdx !== null ? <Pencil size={14} /> : <Plus size={14} />}
																	onClick={addObsToEdit}
																	sx={{ height: 40, borderColor: editingObsInEditIdx !== null ? "#f0ad00" : "primary.main", color: editingObsInEditIdx !== null ? "#f0ad00" : "primary.main", bgcolor: editingObsInEditIdx !== null ? "#fff9e6" : "#e7f1ff", "&:hover": { bgcolor: editingObsInEditIdx !== null ? "#fff3cc" : "#cce0ff" } }}
																>
										...
										return (
											...
											>
													...
													<Box sx={{ flex: 1 }}>
														<Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 0.5 }}>
															...
															{(activity.observation?.length ?? 0) > 0 ? (
																<Box
																	sx={{ display: "inline-flex", alignItems: "center", gap: 0.5, bgcolor: "#f0f4ff", px: 0.75, py: 0.25, borderRadius: 1, flexShrink: 0, marginLeft: "auto" }}
																>
																...
																<Box sx={{ display: "inline-flex", alignItems: "center", gap: 0.5, flexShrink: 0, marginLeft: "auto" }}>
												...
												<Collapse in={expandedObsIdx === idx}>
													<Box sx={{ mt: 1, pt: 1, borderTop: "1px solid #dee2e6", display: "flex", flexDirection: "column", gap: 0.5 }}>
														{(activity.observation || []).map((obs, obsIdx) => (
															<Box key={obsIdx} sx={{ display: "flex", gap: 1 }}>
																<Typography variant="caption" sx={{ color: "#888", minWidth: 70, fontSize: "0.6rem", flexShrink: 0 }}>
																	{new Date(obs.date_observation).toLocaleDateString("pt-BR", { timeZone: "UTC" })}
																</Typography>
										...
										{(newActivity.observation || []).length > 0 && (
											<Box sx={{ display: "flex", flexDirection: "column", gap: 0.5, mb: 1 }}>
												{(newActivity.observation || []).map((obs, obsIdx) => (
													<Box key={obsIdx} sx={{ display: "flex", alignItems: "center", gap: 1, bgcolor: editingObsInNewIdx === obsIdx ? "#fff9e6" : "#f8f9fa", p: 0.75, borderRadius: 1, border: `1px solid ${editingObsInNewIdx === obsIdx ? "#f0ad00" : "#dee2e6"}` }}>
														<Typography variant="caption" sx={{ color: "#666", minWidth: 80, flexShrink: 0 }}>
															{new Date(obs.date_observation).toLocaleDateString("pt-BR", { timeZone: "UTC" })}
														</Typography>
														<Typography variant="caption" sx={{ flex: 1 }}>
														...
														<IconButton size="small" onClick={() => { setEditingObsInNewIdx(obsIdx); setNewObsForNew({ date_observation: obs.date_observation?.split("T")[0] ?? "", text_observation: obs.text_observation }); }} sx={{ color: "primary.main", p: 0.25 }}>
															<Pencil size={12} />
														</IconButton>
														<IconButton size="small" onClick={() => removeObsFromNew(obsIdx)} sx={{ color: "error.main", p: 0.25 }}>
															<Trash2 size={12} />
														</IconButton>
													</Box>
												))}
										...
										<Grid container spacing={1} alignItems="flex-end">
											...
											<Grid item xs={12} sm={2}>
												<Button
													...
													onClick={addObsToNew}
													sx={{ height: 40, borderColor: editingObsInNewIdx !== null ? "#f0ad00" : "primary.main", color: editingObsInNewIdx !== null ? "#f0ad00" : "primary.main", bgcolor: editingObsInNewIdx !== null ? "#fff9e6" : "#e7f1ff", "&:hover": { bgcolor: editingObsInNewIdx !== null ? "#fff3cc" : "#cce0ff" } }}
												>
